...
class CreateDataset_Server(Dataset):
    def __init__(self, database_dir, batch_size=None):
        ...
        self.protein_names = [pdb_f.split('\\')[-1].replace('.pdb','') 
                              for pdb_f in self.pdb_fs]
    
    def __len__(self):
        return len(self.pdb_fs)

    def __getitem__(self, idx):
        # Load data on-demand
        pdb_f = self.pdb_fs[idx]
        protein_ = self.protein_names[idx]
        chain_, seq_ = self.pdb_fasta(pdb_f)
        coord_, _ = load_coords(pdb_f, chain_)
        plddt_ = self.parser_plddt(pdb_f)
        esm_ = self.fasta_esm(seq_)  # Compute when needed
        
        return {'protein': protein_, 'esm1v': esm_, 
                'coords': coord_, 'seq': seq_, 'plddt': plddt_}
    ...
    def fasta_esm(self, fasta):
        ...
        pred = token_embeddings[0].cpu().numpy()[1:]
        return pred
